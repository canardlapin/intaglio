...
object LegendTextLayout:
  def wrap(
  ...
  ): Either[LegendError, LegendTextLayout] =
    if !widthPt.isFinite || widthPt <= 0 || !fontPt.isFinite || fontPt <= 0 || fontFamily.exists(
        _.trim.isEmpty
      )
    then return Left(LegendError.InvalidLayout("text width and font must be positive and finite"))
    ...
    if !height.isFinite || height <= 0 then
      return Left(LegendError.InvalidLayout("text height must be positive and finite"))
    ...
    while index < words.length do
      ...
      if !measured.isFinite || measured < 0 then
        return Left(LegendError.InvalidLayout("text width must be finite and nonnegative"))
      ...
      while rest.nonEmpty && width(rest) > widthPt do
        ...
        if width(rest.take(count)) > widthPt then
          return Left(LegendError.InvalidLayout("a text character exceeds the available width"))
        var next = count
        while next < rest.length && width(
            rest.take(next + Character.charCount(rest.codePointAt(next)))
          ) <= widthPt
        do next += Character.charCount(rest.codePointAt(next))
        count = next
