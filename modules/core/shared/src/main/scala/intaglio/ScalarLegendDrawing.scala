...
/** A measured, renderer-neutral legend in physical points. Text wraps within the card and colliding
  * tick labels occupy separate rows. Bar cells are sampled image strips with nearest-neighbor
  * rendering. Mapping boundaries split the strips, so interpolation cannot smear threshold or gap
  * colors.
  */
object ScalarLegendDrawing:
  def draw(
      legend: ScalarLegend,
      style: ScalarLegendStyle = ScalarLegendStyle(),
      extraNotes: Vector[String] = Vector.empty
  ): Either[LegendError, ScalarLegendDrawing] =
    if extraNotes.exists(_.trim.isEmpty) then
      return Left(LegendError.InvalidMetadata("supplied legend notes must be nonempty"))
    ...
    if !lineHeight.isFinite || lineHeight <= 0 || !width("M").isFinite || width("M") > available
    then return Left(LegendError.InvalidLayout("text metrics do not fit the available width"))
    ...
    val labels = legend.ticks.map: tick =>
      ...
      val left = math.max(
        style.marginPt,
        math.min(center - labelWidth * 0.5, style.widthPt - style.marginPt - labelWidth)
      )
      val found = laneEnds.indexWhere(end => left >= end + style.gapPt)
      val lane = if found >= 0 then found
      else
    ...
    val keys =
      (if legend.showHidden then Vector("Hidden" -> legend.mapping.hidden) else Vector.empty) ++
        (if legend.showInvalid then Vector("Invalid / missing" -> legend.mapping.invalid)
         else Vector.empty)
    ...
    val keyTops = keyLabels.scanLeft(keyTop)((top, lines) =>
      top + math.max(style.barHeightPt, lines.length * lineHeight) + style.gapPt
    )
    val height = keyTops.last + style.marginPt
    val grobs = Vector.newBuilder[Grob]
    def point(x: Double, top: Double): Point =
      Point(LengthExpr(Length.pointsUnsafe(x)), LengthExpr(Length.pointsUnsafe(height - top)))
    def size(w: Double, h: Double): Size =
      Size.fromExtents(ExtentExpr.pointsUnsafe(w), ExtentExpr.pointsUnsafe(h))
    def rgba(color: Rgba32): Rgba =
      Rgba.unsafe(color.red, color.green, color.blue, color.alpha.toDouble / 255)
    val textGp = GraphicParams.unsafe(
      stroke = Some(Rgba.Black),
      fontSize = Length.pointsUnsafe(style.fontPt),
    ...
    def fill(x: Double, top: Double, w: Double, h: Double, color: Rgba32, name: String): Unit =
      grobs += Grob.rectUnsafe(
        ...
        size(w, h),
        anchor = Anchor(HJust.Left, VJust.Top),
        gp = GraphicParams.unsafe(stroke = None, fill = Some(rgba(color))),
        name = Some(GraphicsName.unsafe(name))
      )
    def checker(x: Double, top: Double, w: Double, h: Double, name: String): Unit =
      ...
      for row <- 0 until rows; col <- 0 until columns do
        val color = if (row + col) % 2 == 0 then Rgba32.unsafe(235, 235, 235)
        else Rgba32.unsafe(255, 255, 255)
        fill(
          x + col * tile,
          top + row * tile,
          math.min(tile, w - col * tile),
          math.min(tile, h - row * tile),
          color,
          s"$name-$row-$col"
        )
    titleLines.zipWithIndex.foreach((line, i) =>
      textLine(line, style.marginPt, style.marginPt + i * lineHeight, s"legend-title-$i")
    )
    ...
    val boundaries = (Vector(0.0, 1.0) ++ legend.mapping.boundaries
      .filter(v => v >= legend.mapping.scale.window.lower && v <= legend.mapping.scale.window.upper)
      .map(legend.position)).distinct.sorted
    boundaries
      .sliding(2)
      .zipWithIndex
      .foreach: (pair, i) =>
        ...
        grobs += Grob.imageUnsafe(
          image,
          point(style.marginPt + lo * available, barTop),
          size(span * available, style.barHeightPt),
          anchor = Anchor(HJust.Left, VJust.Top),
          interpolation = RasterInterpolation.Nearest,
          name = Some(GraphicsName.unsafe(s"legend-strip-$i"))
        )
    grobs += Grob.rectUnsafe(
      point(style.marginPt, barTop),
      size(available, style.barHeightPt),
      anchor = Anchor(HJust.Left, VJust.Top),
      gp = GraphicParams.unsafe(stroke = Some(Rgba.Black), fill = None),
    ...
    val placed = labels.zipWithIndex.map: (label, i) =>
      ...
      grobs += Grob
        .segments(
          Vector(point(x, barBottom) -> point(x, barBottom + style.gapPt)),
          textGp,
          name = Some(GraphicsName.unsafe(s"legend-tick-$i"))
        )
        .orThrow
      lines.zipWithIndex.foreach((line, n) =>
        textLine(line, left, top + n * lineHeight, s"legend-label-$i-$n")
      )
      ScalarLegendTickPlacement(
        tick.value,
        fraction,
        tick.label,
        left,
    ...
    notes.zipWithIndex.foreach((line, i) =>
      textLine(line, style.marginPt, noteTop + i * lineHeight, s"legend-note-$i")
    )
    keys.zipWithIndex.foreach: (key, i) =>
      ...
      keyLabels(i).zipWithIndex.foreach((line, n) =>
        textLine(
          line,
          style.marginPt + style.barHeightPt + style.gapPt,
          top + n * lineHeight,
          s"legend-key-label-$i-$n"
        )
      )
    Right(
      ScalarLegendDrawing(Scene(grobs.result()), style.widthPt, height, placed, legend.canonicalKey)
    )
