...
/** Mapping-owned calibration, independent of camera, compositing, and layout. Tick positions use
  * the full scalar window; diverging and split segments therefore retain their actual asymmetric
  * widths. Color evaluation uses each segment's own normalization, exactly as on the mapped
  * scientific object.
  */
final class ScalarLegend private (
  ...
  def position(value: Double): Double =
    require(
      value.isFinite && value >= mapping.scale.window.lower && value <= mapping.scale.window.upper,
  ...
  /** Piecewise-constant display cells. No cell crosses a mapping discontinuity; each cell's color
    * is the exact evaluator output at its recorded sample. Pointwise endpoint inclusion remains
    * available through mapping.color.
    */
  def cells(resolution: Int): Vector[ScalarLegendCell] =
    ...
    val breaks = mapping.boundaries
      .filter(v => v >= mapping.scale.window.lower && v <= mapping.scale.window.upper)
      .map(position)
  ...
  def notes: Vector[String] =
    def interval(value: ScalarInterval): String =
      ...
      val left =
        if value.endpoints == ScalarEndpointInclusion.Lower || value.endpoints == ScalarEndpointInclusion.Both
        then "["
        else "("
      val right =
        if value.endpoints == ScalarEndpointInclusion.Upper || value.endpoints == ScalarEndpointInclusion.Both
        then "]"
        else ")"
  ...
  lazy val canonicalKey: String =
    ...
    def number(value: Double): String =
      java.lang.Long.toHexString(java.lang.Double.doubleToLongBits(if value == 0 then 0 else value))
...
object ScalarLegend:
  private[intaglio] def labels(values: Vector[Double]): Vector[String] =
    ...
    if formatted.distinct.length == values.distinct.length then formatted
    else values.map(value => BigDecimal.decimal(value).bigDecimal.stripTrailingZeros.toPlainString)

  def make(
      mapping: ScalarMapping,
      title: LegendTitle,
      ticks: Vector[AxisTick] = Vector.empty,
      showHidden: Boolean = true,
      showInvalid: Boolean = true
  ): Either[LegendError, ScalarLegend] =
    ...
    val selected = if ticks.isEmpty then
      values.zip(defaultLabels).map((v, label) => AxisTick.unsafe(v, label))
    else ticks.sortBy(_.value)
    if selected.exists(t =>
        !t.value.isFinite || t.value < window.lower || t.value > window.upper || t.label.trim.isEmpty
      )
    then Left(LegendError.InvalidTicks("ticks require finite in-range values and nonempty labels"))
