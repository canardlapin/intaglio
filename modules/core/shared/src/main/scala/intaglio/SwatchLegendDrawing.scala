...
final case class SwatchLegendPlacement(
    label: String,
    color: Rgba32,
    topPt: Double,
    heightPt: Double
)
final case class SwatchLegendDrawing(
    scene: Scene,
    widthPt: Double,
    heightPt: Double,
    entries: Vector[SwatchLegendPlacement]
)

/** Generic categorical or manual color key. Long labels wrap beside their swatch; translucent
  * colors are drawn over a checkerboard.
  */
object SwatchLegendDrawing:
  def draw(
      title: LegendTitle,
      entries: Vector[(String, Rgba32)],
      notes: Vector[String] = Vector.empty,
      style: SwatchLegendStyle = SwatchLegendStyle()
  ): Either[LegendError, SwatchLegendDrawing] =
    ...
    yield
      ...
      val tops = labels.scanLeft(firstTop)((top, label) =>
        top + math.max(style.swatchPt, label.heightPt) + style.gapPt
      )
      ...
      def text(layout: LegendTextLayout, x: Double, top: Double, name: String): Unit =
        layout.lines.zipWithIndex.foreach: (line, i) =>
          grobs += Grob.textUnsafe(
            line,
            point(x, top + i * layout.lineHeightPt),
            anchor = Anchor(HJust.Left, VJust.Top),
            gp = gp,
            name = Some(GraphicsName.unsafe(s"$name-$i"))
          )
      def fill(x: Double, top: Double, w: Double, color: Rgba32, name: String): Unit =
        grobs += Grob.rectUnsafe(
          point(x, top),
          Size.fromExtents(ExtentExpr.pointsUnsafe(w), ExtentExpr.pointsUnsafe(w)),
          anchor = Anchor(HJust.Left, VJust.Top),
      ...
      val placed = entries.zipWithIndex.map: (entry, i) =>
        ...
        for row <- 0 until 2; col <- 0 until 2 do
          ...
          fill(
            style.marginPt + col * style.swatchPt / 2,
            top + row * style.swatchPt / 2,
            style.swatchPt / 2,
            color,
            s"swatch-checker-$i-$row-$col"
          )
        ...
        SwatchLegendPlacement(
          entry._1.trim,
          entry._2,
          top,
          math.max(style.swatchPt, labels(i).heightPt)
        )
      noteLines.zipWithIndex.foreach((note, i) =>
        text(note, style.marginPt, noteTops(i), s"swatch-note-$i")
      )
      SwatchLegendDrawing(Scene(grobs.result()), style.widthPt, height, placed)

  private def traverse[A, B](
      values: Vector[A]
  )(f: A => Either[LegendError, B]): Either[LegendError, Vector[B]] =
