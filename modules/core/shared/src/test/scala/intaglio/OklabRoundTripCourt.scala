...
/** Exhaustive court for the claim that `Oklab.toRgba(Oklab.fromRgba(color))` returns `color` for
  ...
  * run. Run it on demand, on either platform, and record the result:
  *
  * {{{
  * sbt "coreJVM/Test/runMain intaglio.oklabRoundTripCourt"
  * sbt "coreJS/Test/runMain intaglio.oklabRoundTripCourt"
  * }}}
  *
  * It lives in the shared sources rather than the JVM ones so the Scala.js run is possible at all.
  * `Oklab` puts `math.cbrt` and `math.pow` between a caller's colour and the bytes a backend
  * receives, and both are implementation-approximated in JavaScript, so "every colour round trips"
  * is a claim that has to be made per platform rather than inferred from one of them.
  *
  * `evidence/diverging-palette/README.md` holds the receipts from the last runs.
  */
