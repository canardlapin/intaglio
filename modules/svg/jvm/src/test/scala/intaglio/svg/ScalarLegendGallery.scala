...
object ScalarLegendGallery:
  def main(args: Array[String]): Unit =
    ...
    val missing = diverging.copy(
      visibility = ScalarVisibility.Outside(
        ScalarInterval.make(-0.25, 0.25, ScalarEndpointInclusion.Neither).toOption.get
      ),
      invalid = Rgba32.unsafe(190, 60, 175)
    )
    for
      (name, mapping) <- Vector(
        "sequential" -> sequential,
        "asymmetric-diverging" -> diverging,
        "split" -> split,
        "missing-threshold" -> missing
      )
      (format, style) <- Vector(
        "manuscript" -> ScalarLegendStyle(),
        "poster" -> ScalarLegendStyle(widthPt = 440, fontPt = 18, barHeightPt = 26)
      )
    do
      val title = LegendTitle
        .make(
          "Estimated task-related response relative to the comparison condition",
          Some("percent signal change")
        )
        .toOption
        .get
      ...
      val options = SvgOptions.unsafe(
        math.ceil(drawing.widthPt * 4 / 3).toInt,
        math.ceil(drawing.heightPt * 4 / 3).toInt,
        Some(name)
      )
